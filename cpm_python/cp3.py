from cpm_python import calculation
#import cpm_python.calculation
from cpm_python import cpm
from cpm_python import cpm2
...
def cpm_main():
    ...
    if st.button('ok'):
        ...
        try: 
            f = open("/cpm_python/tmp/cpm.txt", "w")
        except:
            f = open("cpm_python/tmp/cpm.txt", "w")
        for i in range(n):
            a=f'{i}'
        ...
        f.close()
        try:
            mydic=calculation.show_results('/cpm_python/tmp/cpm.txt')
        except:
            mydic=calculation.show_results('cpm_python/tmp/cpm.txt')
        tasks=mydic
        #print(mydic)
        ...
        cl1,cl2,cl3,cl4,cl5,cl6,cl7,cl8=st.beta_columns(8)
        with cl1:
